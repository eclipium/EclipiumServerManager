...
public static class AppsBackups
{
    ...
    public static void Restore(string[] args)
    {
        ...
        if (File.Exists("/home/eclipium-server-manager/backups/" + args[1]))
        {
            var serviceName = (new Regex(@"(.+?)(?=-[\d]{2}-[\d]{2}-[\d]{4}-[\d]{2}:[\d]{2}.tar.gz)")).Match(args[1])
                .Value;
            var apps = AppsInformations.GetAppsAsList();
            foreach (var app in apps)
            {
                ...
                {
                    ProcessManager.RunCommandWithBash(
                        $"rm -rf /home/eclipium-server-manager/applications/{serviceName}");
                    ProcessManager.RunCommandWithBashNoOutput(
                        $"tar -xf /home/eclipium-server-manager/backups/{args[1]} -C /home/eclipium-server-manager/applications/");
    ...
    public static void ListBackups(string[] args)
    {
        ...
        foreach (var backupPath in backupsList)
        {
            ...
            var serviceName = (new Regex(@"(.+?)(?=-[\d]{2}-[\d]{2}-[\d]{4}-[\d]{2}:[\d]{2}.tar.gz)")).Match(backup)
                .Value;
                ...
                foreach (var app2 in apps)
                {
                    if (app2.ServiceName == serviceName)
                    {
                        table.AddRow(app2.DisplayName, app2.ServiceName,
                            ...
                            (new Regex(@"(?<=[\d]{2}-[\d]{2}-[\d]{4}-)([\d]{2}:[\d]{2})(?=.tar.gz)")).Match(backup)
                            .Value,
                            backup);
                    }
                }
    ...
    {
        ProcessManager.RunCommandWithBashNoOutput(
            $"tar -czvf {tgzFilename} --directory=/home/eclipium-server-manager/applications {folderName}");
    }
}